    ...
    for (int k = 0; k < nodes1D_; ++k) {
      for (int j = 0; j < numQuad_; ++j) {
        for (int i = 0; i < numQuad_; ++i) {
          ...
          intgLocShift_[vector_index_2D + 1] =
...
//--------------------------------------------------------------------------
double
Quad93DSCS::gauss_point_location(int nodeOrdinal, int gaussPointOrdinal) const
{
...
{
  for (int ip = 0; ip < numIntPoints; ++ip) {
    const int grad_offset = surfaceDimension_ * nodesPerElement_ *
                            ip; // nodes per element is always 9
  ...
  const double* elemNodalCoord, const double* pointCoord, double* isoParCoord)
{
  ...
  double y[3] = {
    elemNodalCoord[10] - elemNodalCoord[9],
    elemNodalCoord[11] - elemNodalCoord[9],
    elemNodalCoord[12] - elemNodalCoord[9]};

  double z[3] = {
    elemNodalCoord[19] - elemNodalCoord[18],
  ...
  do {
    ...
    j[0] = 0.125 * (-2. * (-1. + solcur[1]) * x[0] +
                    (2. * (1. + solcur[1]) * (x[1] - x[2]) +
                     solcur[2] * (-(y[1] * z[0]) + y[2] * z[0] + y[0] * z[1] -
                                  y[0] * z[2])));

    j[1] =
      0.125 * (-2. * (1. + solcur[0]) * x[0] + 2. * (1. + solcur[0]) * x[1] -
               2. * (-1. + solcur[0]) * x[2] +
    ...
    j[3] =
      0.125 *
      (-2. * (-1. + solcur[1]) * y[0] +
       (2. * (1. + solcur[1]) * (y[1] - y[2]) +
        solcur[2] * (x[1] * z[0] - x[2] * z[0] - x[0] * z[1] + x[0] * z[2])));

    j[4] =
      0.125 * (-2. * (1. + solcur[0]) * y[0] + 2. * (1. + solcur[0]) * y[1] -
               2. * (-1. + solcur[0]) * y[2] +
    ...
    j[6] = 0.125 *
           ((solcur[2] *
            ...
            2. * ((-1. + solcur[1]) * z[0] - (1. + solcur[1]) * (z[1] - z[2])));

    j[7] =
      0.125 * ((solcur[2] * (x[2] * (y[0] - y[1]) + (-x[0] + x[1]) * y[2])) -
               2. * (1. + solcur[0]) * z[0] + 2. * (1. + solcur[0]) * z[1] -
               2. * (-1. + solcur[0]) * z[2]);
  ...
  if (i < MAX_NR_ITER) {
    ...
    // Rescale the distance vector by the length of the (non-unit) normal
    // vector, which was used above in the NR iteration.
    ...
    const std::array<double, 3> xtmp = {
      {isoParCoord[0], isoParCoord[1], isoParCoord_2}};
...
Quad93DSCS::non_unit_face_normal(
  const double* isoParCoord, const double* elemNodalCoord, double* normalVector)
{
  ...
  const double n0 = 0.125 * (xi * y[2] * z[0] + y[0] * z[1] + xi * y[0] * z[1] -
                             y[2] * z[1] - xi * y[0] * z[2] +
                             y[1] * (-((1. + xi) * z[0]) + (1. + eta) * z[2]) +
                             eta * (y[2] * z[0] - y[2] * z[1] - y[0] * z[2]));

  const double n1 =
    0.125 *
    (-(xi * x[2] * z[0]) - x[0] * z[1] - xi * x[0] * z[1] + x[2] * z[1] +
     xi * x[0] * z[2] + x[1] * ((1. + xi) * z[0] - (1. + eta) * z[2]) +
     eta * (-(x[2] * z[0]) + x[2] * z[1] + x[0] * z[2]));

  const double n2 = 0.125 * (xi * x[2] * y[0] + x[0] * y[1] + xi * x[0] * y[1] -
                             x[2] * y[1] - xi * x[0] * y[2] +
                             x[1] * (-((1. + xi) * y[0]) + (1. + eta) * y[2]) +
...
//--------------------------------------------------------------------------
double
Quad93DSCS::parametric_distance(const std::array<double, 3>& x)
{
