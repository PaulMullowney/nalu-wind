...
//--------------------------------------------------------------------------
double
HexahedralP2Element::parametric_distance(const std::array<double, 3>& x)
{
...
//--------------------------------------------------------------------------
double
HexahedralP2Element::isInElement(
  const double* elemNodalCoord, const double* pointCoord, double* isoParCoord)
{
  ...
  do {
    ...
    error_vec[2] =
      pointCoord[2] - ddot(weights.data(), elemNodalCoord + 2 * nNodes, nNodes);
  ...
  } while (!within_tolerance(vecnorm_sq3(delta.data()), isInElemConverged) &&
           (++iter < N_MAX_ITER));
  ...
  int s1Node, int s2Node, int s3Node, int s1Ip, int s2Ip, int s3Ip) const
{
  ...
  const double weight = isoparametricArea * gaussWeight_[s1Ip] *
                        gaussWeight_[s2Ip] * gaussWeight_[s3Ip];
...
HexahedralP2Element::hex27_shape_deriv(
  int numIntPoints, const double* intgLoc, double* shapeDerivs) const
{
  const double half = 1.0 / 2.0;
  const double one4th = 1.0 / 4.0;
  const double one8th = 1.0 / 8.0;
...
Hex27SCV::Hex27SCV() : HexahedralP2Element()
{
  ...
  interpWeights_ =
    copy_interpolation_weights_to_view<InterpWeightType>(shapeFunctions_);
  shiftedInterpWeights_ =
    copy_interpolation_weights_to_view<InterpWeightType>(shapeFunctionsShift_);

  referenceGradWeights_ =
    copy_deriv_weights_to_view<GradWeightType>(shapeDerivs_);
  shiftedReferenceGradWeights_ =
    copy_deriv_weights_to_view<GradWeightType>(shapeDerivsShift_);
#endif
}
        ...
        for (int k = 0; k < numQuad_; ++k) {
          for (int j = 0; j < numQuad_; ++j) {
            for (int i = 0; i < numQuad_; ++i) {
              ...
              intgLocShift_[vector_index + 1] =
                shifted_gauss_point_location(m, j);
              intgLocShift_[vector_index + 2] =
                shifted_gauss_point_location(n, k);
...
Hex27SCV::jacobian_determinant(
  const double* elemNodalCoords, const double* shapeDerivs) const
{
  double dx_ds1 = 0.0;
  double dx_ds2 = 0.0;
  double dx_ds3 = 0.0;
...
//--------------------------------------------------------------------------
void
Hex27SCV::Mij(const double* coords, double* metric, double* deriv)
{
...
Hex27SCS::Hex27SCS() : HexahedralP2Element()
{
  ...
  eval_shape_derivs_at_face_ips();
  expReferenceGradWeights_ =
    copy_deriv_weights_to_view<ExpGradWeightType>(expFaceShapeDerivs_);
#endif
}
      ...
      for (int k = 0; k < nodes1D_; ++k) {
        ...
        for (int j = 0; j < numQuad_; ++j) {
          for (int i = 0; i < numQuad_; ++i) {
            ...
            intgLocShift_[vector_index + 0] =
              shifted_gauss_point_location(k, i);
            intgLocShift_[vector_index + 1] =
              shifted_gauss_point_location(l, j);
            intgLocShift_[vector_index + 2] = scsLoc[m];

            // compute the quadrature weight
            ipInfo_[scalar_index].weight =
              orientation[m] * tensor_product_weight(k, l, i, j);
    ...
    for (int k = 0; k < nodes1D_; ++k) {
      ...
      for (int j = 0; j < numQuad_; ++j) {
        for (int i = 0; i < numQuad_; ++i) {
          ...
          // integration point location
          intgExpFace_[vector_index] =
            intgLoc_[oppFace_[scalar_index] * nDim_ + 0];
          intgExpFace_[vector_index + 1] = faceLoc[faceOrdinal];
          intgExpFace_[vector_index + 2] =
            intgLoc_[oppFace_[scalar_index] * nDim_ + 2];
  ...
  for (int faceIp = 0; faceIp < traits::numFaceIp_; ++faceIp) {
    for (unsigned npe = 0; npe < expReferenceGradWeights_.extent(1); ++npe) {
      for (unsigned d = 0; d < expReferenceGradWeights_.extent(2); ++d) {
        deriv(faceIp, npe, d) =
          expReferenceGradWeights_(offset + faceIp, npe, d);
      }
    }
  }
  ...
  double* det_j) const
{
  ...
  *det_j = dx_ds1 * (dy_ds2 * dz_ds3 - dz_ds2 * dy_ds3) +
  ...
  for (int node = 0; node < AlgTraits::nodesPerElement_; ++node) {
    ...
    grad[vector_offset + 0] =
      dn_ds1 * ds1_dx + dn_ds2 * ds2_dx + dn_ds3 * ds3_dx;
    grad[vector_offset + 1] =
      dn_ds1 * ds1_dy + dn_ds2 * ds2_dy + dn_ds3 * ds3_dy;
    grad[vector_offset + 2] =
      dn_ds1 * ds1_dz + dn_ds2 * ds2_dz + dn_ds3 * ds3_dz;
  }
}
...
//--------------------------------------------------------------------------
void
Hex27SCS::Mij(const double* coords, double* metric, double* deriv)
{
  generic_Mij_3d<AlgTraits>(numIntPoints_, deriv, coords, metric);
}
//--------------------------------------------------------------------------
  ...
  double* error)
{
  ...
  std::array<double, AlgTraits::nodesPerElement_ * AlgTraits::nDim_>
    faceShapeFuncDerivs;

  hex27_shape_deriv(ipsPerFace, isoParCoord, faceShapeFuncDerivs.data());

  gradient(coords, faceShapeFuncDerivs.data(), gradop, det_j);
