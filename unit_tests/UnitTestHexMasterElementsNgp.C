...
poly_val(const Kokkos::Array<double, ORD + 1>& coeffs, double x)
{
  double val = 0.0;
...
KOKKOS_FUNCTION double
poly_val(
  const Kokkos::Array<Kokkos::Array<double, ORD + 1>, DIM>& coeffs,
  const double* x)
{
  if (coeffs.size() == 2) {
  ...
  return (
    poly_val<ORD>(coeffs[0], x[0]) * poly_val<ORD>(coeffs[1], x[1]) *
    poly_val<ORD>(coeffs[2], x[2]));
}

template <int ORD>
KOKKOS_FUNCTION double
poly_der(const Kokkos::Array<double, ORD + 1>& coeffs, double x)
{
  double val = 0.0;
  for (unsigned j = 0; j < coeffs.size(); ++j) {
  ...
  int dir)
{
  ...
  if (coeffs.size() == 3) {
    ...
    case 1:
      ...
      break;
    case 2:
      val = poly_val<ORD>(coeffs[0], x[0]) * poly_val<ORD>(coeffs[1], x[1]) *
            poly_der<ORD>(coeffs[2], x[2]);
      break;
    default:
      printf("invalid direction");
      NGP_ThrowAssert(true);
...
void
check_interpolation(
  const stk::mesh::MetaData& meta, const stk::mesh::BulkData& bulk)
{
  ...
  for (int j = 0; j < num_int_pt; ++j) {
    polyResult[j] =
      poly_val<dim, poly_order>(coeffs, &me->integration_locations()[j * dim]);
  }
  ...
  Kokkos::View<DoubleType*, sierra::nalu::MemSpace> ngpResults(
    "ngpResults", num_int_pt);
  Kokkos::View<DoubleType*, sierra::nalu::MemSpace>::HostMirror hostResults =
  ...
  const int bytes_per_thread =
    sierra::nalu::SharedMemView<DoubleType**>::shmem_size(
      num_int_pt, num_nodes);
  ...
  const stk::mesh::MetaData& meta, const stk::mesh::BulkData& bulk)
{
  ...
  for (int j = 0; j < num_int_pt; ++j) {
    for (unsigned d = 0; d < dim; ++d) {
      polyResult[j][d] = poly_der<dim, poly_order>(
        coeffs, &me->integration_locations()[j * dim], d);
    }
  }
  ...
  const int bytes_per_thread =
    ...
    2 * sierra::nalu::SharedMemView<DoubleType***>::shmem_size(
          num_int_pt, num_nodes, dim);
  ...
  Kokkos::parallel_for(
    team_exec, KOKKOS_LAMBDA(const sierra::nalu::DeviceTeamHandleType& team) {
      ...
      sierra::nalu::SharedMemView<DoubleType***, ShmemType> meGrad =
        sierra::nalu::get_shmem_view_3D<DoubleType, TeamType, ShmemType>(
          team, num_int_pt, num_nodes, dim);
  ...
  for (int j = 0; j < num_int_pt; ++j) {
    for (unsigned d = 0; d < dim; ++d) {
      EXPECT_NEAR(
        stk::simd::get_data(hostResults(j, d), 0), polyResult[j][d], 1.0e-12);
    }
  }
  sierra::nalu::MasterElementRepo::clear();
}
...
TEST_F(MasterElementHexSerialNGP, hex8_scs_interpolation)
{
  if (stk::parallel_machine_size(comm) == 1) {
    ...
    check_interpolation<AlgTraits, AlgTraits::masterElementScs_, true>(
      *meta, *bulk);
  }
}
...
TEST_F(MasterElementHexSerialNGP, hex8_scv_interpolation)
{
  if (stk::parallel_machine_size(comm) == 1) {
    setup_poly_order_1_hex_8();
    using AlgTraits = sierra::nalu::AlgTraitsHex8;
    check_interpolation<AlgTraits, AlgTraits::masterElementScv_, false>(
      *meta, *bulk);
  }
}
