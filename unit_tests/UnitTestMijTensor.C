...
TEST(MijTensorNGP, hex27)
{
  ...
  Kokkos::View<double***> mij_tensor(
    "mij_tensor", AlgTraits::numScsIp_, dim, dim);
  ...
  for (int ip = 0; ip < mescs.num_integration_points(); ++ip) {
    for (int d_outer = 0; d_outer < 3; ++d_outer) {
      for (int d_inner = 0; d_inner < 3; ++d_inner) {
        EXPECT_NEAR(
          mij_tensor(ip, d_outer, d_inner), mij_exact[d_outer][d_inner], tol);
      }
    }
  }
}
...
TEST(MijTensorNGP, hex27_simd)
{
  ...
  const auto& coordField =
    *static_cast<const VectorFieldType*>(meta.coordinate_field());
  Kokkos::View<DoubleType**> v_coords(
    "coords", AlgTraits::nodesPerElement_, dim);
  const auto* nodes = bulk->begin_nodes(elem);
  for (unsigned j = 0; j < topo.num_nodes(); ++j) {
    const double* coords = stk::mesh::field_data(coordField, nodes[j]);
  ...
  Kokkos::View<DoubleType***> mij_tensor(
    "mij_tensor", AlgTraits::numScsIp_, dim, dim);
  ...
  for (int ip = 0; ip < mescs.num_integration_points(); ++ip) {
    for (int d_outer = 0; d_outer < 3; ++d_outer) {
      for (int d_inner = 0; d_inner < 3; ++d_inner) {
        EXPECT_NEAR(
          stk::simd::get_data(mij_tensor(ip, d_outer, d_inner), 0),
